<template>
  <el-container id="device_manage" v-loading="loading">
        ...
        <el-col :span="24">
          <div class="grid-content bg-purple-dark">
            <el-row :gutter="20">
              ...
              <el-col :span="1.5">
                <el-button class="spacing20" type="primary" @click="search_block" icon="el-icon-search" round>设备搜索</el-button>
              </el-col>
            </el-row>
          </div>
        </el-col>
    ...
    <el-drawer size="70%"
               ...
               :direction="direction"
               :visible.sync="block_transfer_drawer">
...
<script>
  import { search_block, search_block_hash, block_hash_info, block_info } from "@/api/block"
    ...
    export default {
        ...
        methods: {
            ...
            history_transfer(row){
                search_block_hash(10, row.chipId).then(res => {
                    if (res.success){
            ...
            show_block_info(hash){
                ...
                block_hash_info(hash).then(res => {
                    ...
                    if (res.success) {
                        ...
                        this.block_info_table = [{"name": "hash", "v": res.data.result.hash},
                            {"name": "出款地址", "v": res.data.result.from}, {"name": "收款地址", "v": res.data.result.to},
                            {"name": "区块hash", "v": res.data.result.blockHash}, {"name": "交易时间", "v": date},
                            ...
                            {"name": "数据", "v": res.data.result.input}, {"name": "数据翻译", "v": content}]
                    }else {
                        this.$message({
                            type: 'info',
                            message: res.message
