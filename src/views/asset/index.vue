<template>
  <el-container id="assets_manage" v-loading="loading">
    ...
    <el-drawer
            ...
            :visible.sync="add_assets_drawer"
            :direction="direction">
    ...
    <el-drawer size="70%"
               ...
               custom-class="drawer_class"
               :direction="direction"
               :visible.sync="assets_info_drawer">
    ...
    <!--分配资产-->
    <el-drawer size="70%"
               :title="customer_id === '' || customer_id == null ? '分配资产' : '添加资产'"
               custom-class="drawer_class"
               :direction="direction"
               :visible.sync="assets_distribution_drawer">
...
<script>
    ...
    import { add_asset, del_asset, update_asset, search_asset, public_asset, dis_asset, delDis_asset, asset_types } from "@/api/asset"
    ...
    export default {
        ...
        methods: {
            ...
            assets_info(row) {
                ...
                this.show_table_data = [{k: '名称', v : row.name}, {k: '类型', v: row.type}, {k: '标签', v: row.label} , {k: '说明', v: row.additionalInfo.description}];
            },
            del_assets(id){
                this.$confirm('确定删除该资产, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    del_asset({'assetId': id.id}).then(res=>{
                        if (res.success){
                            this.search_assets();
                            this.$message({
                                type: 'success',
            ...
            add_assets(){
                this.$refs['add_assets_data'].validate((valid) => {
                    if (valid) {
                        this.loading = true;
                        add_asset({'name': this.add_assets_data.name, 'type': this.add_assets_data.type, 'label': this.add_assets_data.label, 'additionalInfo': {'description': this.add_assets_data.description}}).then((res)=>{
                            if (res.success){
                                this.search_assets();
            ...
            distribution_assets(){
                dis_asset({"assetsId": this.assets_distribution_data.assets_id, "customerId": this.assets_distribution_data.customer_id}).then(res => {
                    if (res.success){
                        this.$message({
                            type: 'success',
                            message: '资产分配成功'
            ...
            del_distribution(id){
                delDis_asset({"assetsId": id}).then(res => {
                    if (res.success){
                        this.$message({
                            type: 'success',
                            message: '操作成功'
                    ...
                    if (valid) {
                        update_asset(this.update_data).then((res)=>{
                            if (res.success){
                                ...
                                this.show_table_data = [{k: '名称', v : data.name}, {k: '类型', v: data.type}, {k: '标签', v: data.label} , {k: '说明', v: ati ? ati.description : null}];
            ...
            public_asset(id){
                public_asset({"assetId": id}).then(res => {
                    if (res.success){
                        this.$message({
                            type: 'success',
                            message: '资产已公开'
