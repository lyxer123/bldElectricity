<template>
  <el-container id="device_manage"
        ...
        <el-col :span="24">
          <div class="grid-content bg-purple-dark">
            <el-row :gutter="20">
              ...
              <el-col :span="1.5">
                <el-button class="spacing20"
                           type="primary"
                           @click="search_device"
                           icon="el-icon-search"
                           round>设备搜索</el-button>
              </el-col>
              <el-col :span="1.5"
                      v-if="!isCustomer">
                <el-button @click="get_types(true)"
                           class="spacing20"
                           icon="el-icon-plus"
    ...
    <el-main>
      ...
      <el-table :data="table_data"
        ...
        <el-table-column label="设备ID"
                         prop="id.id"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button class="round_button"
                       @click="info_device(scope.row)"
                       title="详细信息">
              <svg class="bld_icon"
                ...
                <path d="M828.5 886.3H238.4c-9.9 0-18-8.1-18-18v-4c0-9.9 8.1-18 18-18h590.1c9.9 0 18 8.1 18 18v4c0 9.9-8.1 18-18 18zM828.5 684.6H238.4c-9.9 0-18-8.1-18-18v-4c0-9.9 8.1-18 18-18h590.1c9.9 0 18 8.1 18 18v4c0 9.9-8.1 18-18 18zM409 449.5H230.2c-17.6 0-32-14.4-32-32V238.7c0-17.6 14.4-32 32-32H409c17.6 0 32 14.4 32 32v178.8c0 17.6-14.4 32-32 32z m-170.8-40H401V246.7H238.2v162.8zM822.4 437.5H552.9c-11 0-20-9-20-20s9-20 20-20h269.5c11 0 20 9 20 20s-9 20-20 20zM822.4 269.5H552.9c-11 0-20-9-20-20s9-20 20-20h269.5c11 0 20 9 20 20s-9 20-20 20z"
                      fill="#888888"></path>
                <path d="M884.4 1024H182.5c-46.3 0-83.9-37.7-83.9-83.9V91.9C98.6 45.7 136.3 8 182.5 8h701.9c46.3 0 83.9 37.7 83.9 83.9V940c0 46.3-37.7 84-83.9 84zM182.5 48c-24.2 0-43.9 19.7-43.9 43.9V940c0 24.2 19.7 43.9 43.9 43.9h701.9c24.2 0 43.9-19.7 43.9-43.9V91.9c0-24.2-19.7-43.9-43.9-43.9H182.5z"
                      fill="#888888"></path>
              </svg>
            </el-button>
            <template v-if="!isCustomer">
              <el-button class="round_button"
                         @click="public_device(scope.row.id.id)"
                         v-if="scope.row.customerId.id === '13814000-1dd2-11b2-8080-808080808080'"
                         title="公开设备">
                <svg class="bld_icon"
                     viewBox="0 0 1024 1024"
                     version="1.1"
              ...
              <el-button class="round_button"
                         @click="distribution_button(scope.row.id.id)"
                         v-if="scope.row.customerId.id === '13814000-1dd2-11b2-8080-808080808080'"
                         title="分配设备">
                <svg t="1591670629977"
                     class="bld_icon"
              ...
              <el-button class="round_button"
                ...
                <svg class="bld_icon"
                     ...
                     width="200"
                     height="200">
                  <path d="M967.111 227.556H739.556V56.889H284.444v170.667H56.89v56.888h113.778v682.667h682.666V284.444h113.778v-56.888zM341.333 113.778h341.334v113.778H341.333V113.778z m455.111 796.444H227.556V284.444h568.888v625.778z"
                        fill="#73777A"
                        p-id="2331"></path>
                  <path d="M398.222 341.333h56.89v512h-56.89z m170.667 0h56.889v512h-56.89z"
                        fill="#73777A"></path>
                </svg>
              </el-button>
            </template>
    ...
    <el-drawer size="70%"
               ...
               :visible.sync="add_device_drawer"
               :direction="direction">
      <el-form status-icon
               ref="add_device_data"
               :model="add_device_data"
               :rules="add_device_rules"
               label-width="80px"
               class="device_add_from">
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="add_device">立即创建</el-button>
    ...
    <el-drawer :with-header="false"
               custom-class="drawer_class"
               :direction="direction"
               :visible.sync="device_info_drawer"
               @close="info_close"
               size="70%">
      <el-button class="close_button"
                 @click="device_info_drawer = false"
                 icon="el-icon-close"
                 title="关闭"></el-button>
      <el-tabs v-model="activeName"
               @tab-click="table_switch"
               class="device_tabs">
        <el-tab-pane label="详情"
          ...
          <div class="device_show_list">
            <div v-if="!update_mode">
              ...
              <el-button class="show_button update_button"
                         @click="get_types(false)"
                         v-if="!isCustomer">修改信息</el-button>
            </div>
            <el-form status-icon
                     ref="update_data"
                     :model="update_data"
              ...
              <el-form-item>
                <el-button class="show_button"
                           @click="update_info">确认修改</el-button>
              </el-form-item>
              <el-form-item>
                <el-button class="show_button"
                           type="primary"
                           @click="update_mode = false">取消修改</el-button>
              </el-form-item>
            </el-form>
          </div>
        </el-tab-pane>
          ...
          <el-container>
              ...
              <el-row>
                <el-col :span="24">
                  <div class="grid-content bg-purple-dark">
                    <el-row :gutter="20">
                      <el-col :span="5">
                        <el-select v-model="attribute_type"
                                   @change="get_attributes"
                                   placeholder="请选择">
                          <el-option v-for="item in attribute_types"
                                     :key="item.value"
                                     :label="item.label"
                      ...
                      <el-col :span="10">
                        <el-input class="search_input"
                                  v-model="attribute_search"
                                  placeholder="请输入搜索信息"
                                  clearable></el-input>
                      </el-col>
                      <el-col :span="1.5">
                        <el-button @click="get_attributes"
                                   class="spacing20"
                                   type="primary"
                                   icon="el-icon-search"
                                   round>搜索</el-button>
                      ...
                      <el-col :span="1.5">
                        <el-button @click="re_attributes"
                                   class="spacing20"
                                   icon="el-icon-refresh-right"
                                   round>刷新</el-button>
                      </el-col>
                      <el-col :span="1.5"
                              v-if="attribute_type === 'SERVER_SCOPE' || attribute_type === 'SHARED_SCOPE'">
                        <el-button @click="add_attribute_button"
                                   class="spacing20"
                                   icon="el-icon-plus"
                                   round>添加</el-button>
                      </el-col>
            ...
            <el-main>
              <el-table ref="attributes_table"
                ...
                <el-table-column prop="lastUpdateTs"
                                 label="时间"
                                 :formatter="lastUpdateTsdataformat">
                ...
                <el-table-column align="left"
                  ...
                  <template slot-scope="scope"
                            v-if="!isCustomer">
                    <el-button class="round_button"
                               @click="update_attribute(scope.row)"
                               title="修改属性"
                               style="color: #606266;">
              ...
              <div v-if="selection.length > 0">
                ...
                <el-button @click="del_attribute"
                           type="danger">删除属性</el-button>
              </div>
            </el-main>
          </el-container>
        </el-tab-pane>
        <el-tab-pane label="遥测"
          ...
          <div style="margin: 1rem 0">
            ...
            <el-table :data="telemetry_hash"
                      style="width: 100%">
              <el-table-column label="txHash"
                               min-width="125">
                <el-link @click="show_block_info(scope.row.txhash)"
                         :underline="false"
                         slot-scope="scope">{{ scope.row.txhash }}</el-link>
              </el-table-column>
    ...
    <!--分配设备-->
    <el-drawer size="70%"
               :title="customerId == null || customerId === '' ? '分配设备' : '添加设备'"
               custom-class="drawer_class"
               :direction="direction"
               :visible.sync="device_distribution_drawer">
      <el-form status-icon
               ref="device_distribution_data"
               :model="device_distribution_data"
               :rules="assets_distribution_rules"
               label-width="80px"
               class="assets_distribution_from">
        <el-form-item label="分配客户"
                      prop="customer_id"
                      v-if="customerId == null || customerId === ''">
          <el-select filterable
                     allow-create
                     v-model="device_distribution_data.customer_id"
                     placeholder="请选择分配客户"
                     size="medium">
        ...
        <el-form-item label="设备列表"
          ...
          <el-select filterable
                     allow-create
                     v-model="device_distribution_data.device_id"
                     placeholder="请选择设备"
                     size="medium">
            <el-option v-for="item in device_list"
                       :label="item.name"
                       :value="item.id.id">
            </el-option>
          </el-select>
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="distribution_device">分配</el-button>
    ...
    <el-dialog :visible.sync="add_attribute_dialog">
      <el-form ref="attribute_from"
               :rules="attribute_from_rules"
               :model="attribute_from"
               label-width="80px">
        <el-form-item label="名称"
                      prop="k">
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="add_attribute">{{update_attribute_boo ? '立即修改' : '立即创建'}}</el-button>
    ...
    <el-drawer size="65%"
               ...
               custom-class="drawer_class"
               :direction="direction"
               :visible.sync="block_info_drawer">
      <div class="assets_show_list"
           v-loading="block_loading">
        <el-table :data="block_info_table"
                  style="width: 100%">
...
<script>
import { add_device, del_device, update_device, search_device, device_types, device_token, public_device, dis_device, del_dis_device, add_device_attributes, del_device_attributes, device_attributes, device_telemetry } from "@/api/device"
import { search_client_ids, search_client } from "@/api/client"
import DataShowInput from "@/components/DataShowInput"
import { ati_deal, date_format, block_dp } from "@/utils/functions";
import { block_hash_info, block_info } from "@/api/block"
...
export default {
  ...
  data () {
    return {
      ...
      attribute_from_rules: {
        ...
        v: [
          { required: true, message: '请输入属性数值', trigger: 'blur' },
        ]
      },
      telemetry_hash: [],
      loading: false,
  ...
  methods: {
    ...
    info_device (row) {
      ...
      this.show_table_data = [{ k: '设备名称', v: row.name }, { k: '设备TOKEN', v: "没有获取到设备token" }, { k: '设备类型', v: row.type }, { k: '设备标签', v: row.label }, { k: '网关设备', v: row.additionalInfo.gateway }, { k: '设备说明', v: row.additionalInfo.description }, { k: '添加时间', v: date_format(row.createdTime) }];
    ...
    del_device (id) {
      this.$confirm('确定删除该设备, 是否继续?', '提示', {
        ...
        del_device({ 'deviceId': id.id }).then(res => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    add_device () {
      this.$refs['add_device_data'].validate((valid) => {
        if (valid) {
          add_device({ 'name': this.add_device_data.name, 'type': this.add_device_data.type, 'gateway': this.add_device_data.gateway, 'description': this.add_device_data.description, 'label': this.add_device_data.label }).then((res) => {
            if (res.success) {
              this.search_device()
            } else {
              this.$message({
    ...
    distribution_button (id) {
      search_client({ 'limit': 50, 'search': '' }).then(res => {
        if (res.success) {
          let data = res.data;
          this.customer_list = [];
          for (let i = 0; i < data.length; i++) {
            this.customer_list[this.customer_list.length] = { "name": data[i].name, "id": data[i].id.id }
          }
    ...
    distribution_device () {
      dis_device({ "deviceId": this.device_distribution_data.device_id, "customerId": this.device_distribution_data.customer_id }).then(res => {
        if (res.success) {
          this.$message({
            type: 'success',
            message: '设备分配成功'
    ...
    public_device (id) {
      public_device({ "deviceId": id }).then(r => {
        if (r.data.success) {
          this.$message({
            type: 'success',
            message: '设备已公开'
    ...
    del_distribution (id) {
      this.$confirm('确定取消该设备的分配, 是否继续?', '提示', {
        ...
        del_dis_device({ "deviceId": id.id }).then(res => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: '取消分配成功'
    ...
    table_switch (t, e) {
      ...
      if (i === "1") {
        ...
        /*生成图表*/
        interval = setInterval(() => { thi.telemetry_chart('EN,IA,PT,UA,RSSI,Txhash', thi.socket_id, thi) }, 5000);
        /*websocket获取最新遥测数据*/
        if (thi.old_socket_id === '' || thi.old_socket_id === null) {
          websocket.send(JSON.stringify({ "tsSubCmds": [{ "cmdId": 2, "entityType": "DEVICE", "scope": "LATEST_TELEMETRY", "entityId": thi.socket_id }], "attrSubCmds": [{ "cmdId": 1, "entityType": "DEVICE", "scope": "CLIENT_SCOPE", "entityId": thi.socket_id }, { "cmdId": 3, "entityType": "DEVICE", "scope": "CLIENT_SCOPE", "entityId": thi.socket_id }], "historyCmds": [] }));
        } else {
          websocket.send(JSON.stringify({ "tsSubCmds": [], "historyCmds": [], "attrSubCmds": [{ "entityType": "DEVICE", "entityId": thi.old_socket_id, "scope": "CLIENT_SCOPE", "cmdId": 3, "unsubscribe": true }] }));
          websocket.send(JSON.stringify({ "tsSubCmds": [], "historyCmds": [], "attrSubCmds": [{ "entityType": "DEVICE", "entityId": thi.socket_id, "scope": "CLIENT_SCOPE", "cmdId": 4 }] }));
          websocket.send(JSON.stringify({ "tsSubCmds": [{ "entityType": "DEVICE", "entityId": thi.socket_id, "scope": "LATEST_TELEMETRY", "cmdId": 5 }], "historyCmds": [], "attrSubCmds": [] }));
          websocket.send(JSON.stringify({ "tsSubCmds": [{ "entityType": "DEVICE", "entityId": thi.old_socket_id, "scope": "LATEST_TELEMETRY", "cmdId": 2, "unsubscribe": true }], "historyCmds": [], "attrSubCmds": [] }))
        }
        let data = null;
        websocket.onmessage = e => {
          ...
          if (j != null && j.data != null) {
            ...
            for (let k in data) {
              ...
              thi.telemetry[thi.telemetry.length] = { "name": k, "time": date_format(data[k][0][0]), "v": data[k][0][1] };
              i++;
            }
          }
        };
    ...
    add_attribute: function () {
      this.$refs['attribute_from'].validate((valid) => {
        if (valid) {
          add_device_attributes({ "id": this.socket_id, "type": this.attribute_type, "attribute": "{\"" + this.attribute_from.k + "\": \"" + this.attribute_from.v + "\"}" }).then((res) => {
            if (res.success) {
    ...
    del_attribute () {
      ...
      this.$confirm('确定删除该属性, 是否继续?', '提示', {
        ...
        del_device_attributes({ "deviceId": this.socket_id, "ks": ks }).then(r => {
          if (r.data.success) {
            this.$message({
              type: 'success',
              message: r.data.message
        ...
        if (res.success && res.data && res.data.EN.length > 0) {
          ...
          if (th && th.length > 0) {
            ...
            for (let i = 0; i < th.length && i < 5; i++) {
              list.push({ "txhash": th[i].value, "time": date_format(th[i].ts), "ts": th[i].ts })
            }
    ...
    charts_data_update (thi, ens, old_data) {
      ...
      if (!old_data || old_data.length < 1) {
        ...
        for (let i = 0, l = ens.length; i < l; i++) {
          let en = ens[i];
          old_data[old_data.length] = { "ts": en.ts, "value": [date_format(en.ts), en.value] };
        }
      } else {
        ...
        for (let i = 0, l = ens.length; i < l; i++) {
          ...
          if (en.ts > last.ts) {
            ...
            old_data.push({ "ts": en.ts, "value": [date_format(en.ts), en.value] })
          }
        }
      }
      return old_data;
    ...
    show_block_info (hash) {
      ...
      block_hash_info(hash).then(res => {
        ...
        if (res.success) {
          ...
          this.block_info_table = [{ "name": "hash", "v": data.hash },
          { "name": "出款地址", "v": data.from }, { "name": "收款地址", "v": data.to },
          { "name": "区块hash", "v": data.blockHash }, { "name": "交易时间", "v": date },
          { "name": "交易区块", "v": blockNum }, { "name": "DP使用", "v": dpUsed },
          { "name": "Gas单价", "v": block_dp(gasPrice) }, { "name": "交易确认", "v": newBlockNum - blockNum },
          { "name": "出资地址历史交易次数", "v": nonce }, { "name": "交易总消耗", "v": block_dp(allUsed) },
          { "name": "数据", "v": data.input }, { "name": "数据翻译", "v": content }]
        } else {
          this.$message({
            type: 'info',
            message: res.message
